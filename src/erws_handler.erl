...
start_link_session([<<"trace">>, Session]) ->
    ...
    [{SessKey, Pid, ?UNDEF, NameSpace}] =
	ets:lookup(?ERWS_LINK, SessKey),%%do not use one session
    ets:insert(?ERWS_LINK,
	       {SessKey, Pid, PidTrace,
		NameSpace}),     %%save all links
...
start_link_session([<<"trace">>, Session,
    ...
    PidTrace = spawn(?MODULE, start_trace_process, []),
    SessKey = binary_to_list(Session),
    [{SessKey, Pid, ?UNDEF, OutNameSpaceL}] =
	ets:lookup(?ERWS_LINK, SessKey),%%do not use one session
    ets:insert(?ERWS_LINK,
	       {SessKey, Pid, PidTrace,
		OutNameSpaceL}),     %%save all links
    ets:insert(?ERWS_LINK, {Pid, PidTrace, main, SessKey}),
    ets:insert(?ERWS_LINK, {PidTrace, Pid, trace, SessKey}),
    PidTrace;
start_link_session([Session, OutNameSpace]) ->
    ...
    OutNameSpaceL = binary_to_list(OutNameSpace),
    Pid = spawn(?MODULE, start_shell_process,
		[OutNameSpaceL, public]),
    SessKey = binary_to_list(Session),
    [] = ets:lookup(?ERWS_LINK,
		    SessKey), %%do not use one session
    ets:insert(?ERWS_LINK,
	       {SessKey, Pid, ?UNDEF, OutNameSpaceL}),
    Pid;
...
websocket_handle({text, Msg}, Req, StatePid) ->
    ?CONSOLE_LOG("~p Received: ~p ~n ~p~n~n",
		 [{?MODULE, ?LINE}, Msg, StatePid]),
    ?CONSOLE_LOG(" Req: ~p ~n", [Req]),
    Res = process_req(StatePid, Msg),
    ?CONSOLE_LOG("~p send back: ~p ~n",
		 [{?MODULE, ?LINE}, {StatePid, Res}]),
    {reply, {text, Res}, Req, StatePid, hibernate};
...
delete_session(StatePid) ->
    case ets:lookup(?ERWS_LINK, StatePid) of
	  ...
	  [{_, _, _, NameSpace}] = ets:lookup(?ERWS_LINK,
					      SessKey),
	  ...
	  exit(StatePid, kill);
      [] ->
	  ?CONSOLE_LOG("~p exception ~p",
		       [{?MODULE, ?LINE}, StatePid]),
	  exit(bad)
    end.
...
process_req(StatePid, Msg) ->
    case ets:lookup(?ERWS_LINK, StatePid) of
	  ...
	  Res = receive
		  {result, get_char} -> <<"get_char">>;
		  {result, read} -> <<"read_term">>;
		  {result, write, Something} -> Something;
		  {result, Result} -> PidTrace ! client_finish, Result
		end,
	  ?CONSOLE_LOG("send back: ~p ~n ~p~n~n",
		       [Res, StatePid]),
	  Res;
      [{StatePid, Pid, trace, SessKey}] ->
...
start_trace_process() -> trace_loop(wait_user, ?UNDEF).

trace_loop(wait_user, ?UNDEF) ->
    ?CONSOLE_LOG(" tracer wait user next or finish: ~p ~n",
		 [{?MODULE, ?LINE}]),
    receive
      {"next", Pid} -> %%first
...
trace_loop(wait_user, BackPid) ->
    ?CONSOLE_LOG("tracer wait user yes/no: ~p ~n",
		 [{?MODULE, ?LINE}]),
    receive
      {Line, Pid} ->
	  ?CONSOLE_LOG("got from user: ~p ~n",
		       [{?MODULE, ?LINE, Line}]),
	  case Line of
	    [$y, $e, $s | _] ->
		BackPid ! next, trace_loop(wait_aim, Pid);
	    _ ->
...
trace_loop(wait_aim, Pid) ->
    ?CONSOLE_LOG("~p tracer wait result of aim ~p ~n",
		 [{?MODULE, ?LINE}, Pid]),
    receive
      {debug_msg, BackPid, Index, Body} ->
	  ?CONSOLE_LOG("~p got debug  msg  ~p ~n",
		       [{?MODULE, ?LINE}, Body]),
	  Msg =
	      io_lib:format("aim ~p call  ~p ? ~n yes or no~n  ",
			    [Index, Body]),
	  Pid ! {result, Msg},
	  trace_loop(wait_user, BackPid);
      {res_msg, BackPid, Index, Body} ->
	  ?CONSOLE_LOG("~p got res  msg  ~p ~n",
		       [{?MODULE, ?LINE}, Body]),
	  Msg = io_lib:format("aim ~p got  ~p ?  ~n  ",
			      [Index, Body]),
	  Pid ! {result, Msg},
	  trace_loop(wait_user, BackPid);
      finish ->
	  ?CONSOLE_LOG("~p got finish msg  ~n",
		       [{?MODULE, ?LINE}]),
	  Pid ! {result, finish},
	  trace_loop(wait_user, ?UNDEF);
      client_finish ->
	  ?CONSOLE_LOG("~p new trace wait  ~n",
		       [{?MODULE, ?LINE}]),
	  Pid ! {result, finish},
	  trace_loop(wait_user, ?UNDEF);
      {Line, Pid} ->
	  ?CONSOLE_LOG("~p ignore msg from user  msg ~p ~n",
		       [{?MODULE, ?LINE}, Line]),
	  trace_loop(wait_aim, Pid);
      Unexpected ->
	  ?CONSOLE_LOG("~p got unexpected msg ~p ~n",
		       [{?MODULE, ?LINE}, Unexpected]),
	  Pid ! {result, <<"No\n">>},
	  trace_loop(wait_user, ?UNDEF)
    end.
...
shell_loop(TreeEts, TraceStatus, Prefix) ->
    ...
    ?CONSOLE_LOG("~p working with  namespace ~p",
		 [{?MODULE, ?LINE}, Prefix]),
    case ets:lookup(TreeEts, next) of
      [{NextPid, next, Type}] ->
	  ?CONSOLE_LOG("~p wait answer from user ~p",
		       [{?MODULE, ?LINE}, NextPid]),
	  ets:delete(TreeEts, next),
	  TraceStatusNew = wait_user_input(Type, TraceStatus,
					   TreeEts, NextPid),
	  shell_loop(TreeEts, TraceStatusNew,
		     check_namespace(TreeEts));
      [] -> %%if we wait new aim
	  ?CONSOLE_LOG("~p wait new aim from user ~p",
		       [{?MODULE, ?LINE}, TreeEts]),
	  receive
		...
		spawn(?MODULE, server_loop,
		      [Code, TreeEts, self(), TracePid,
		       TraceStatus]),%%begin new aim
		NewTraceStatus = wait_result(Back, TreeEts,
					     TraceStatus),
...
%%TODO make it safier
check_namespace(TreeEts) ->
    [{system_record, ?PREFIX, Prefix}] = ets:lookup(TreeEts,
						    ?PREFIX),
    Prefix.
...
wait_user_input(get_char, TraceStatus, TreeEts,
		NextPid) ->
    receive
      {some_code, Back, Some, Trace} ->
	  ?CONSOLE_LOG("~p send ~p to ~p",
		       [{?MODULE, ?LINE}, Some, NextPid]),
	  NextPid ! {char, Some},
	  wait_result(Back, TreeEts, TraceStatus);
      R ->
...
get_help() ->
    <<"This is help...<br/>",
      "Use menu Online IDE above for load your "
      "own code to the memory <br/> ",
      "Terminal commands :<br/> ",
      "<strong>trace_on.</strong>  : turn on "
      "tracer under this terminal<br/> ",
      "<strong>trace_off.</strong>  : turn "
      "off tracer under this terminal<br/> ",
      "<strong>listing.</strong>  : show code "
      " and facts availible in memory <br/> "
      "",
      "<strong>yes.</strong>  : positive answer "
      "to questions of the system <br/> ",
      "<strong>no.</strong>  : negative answer "
      "to questions of the system <br/> ",
      "<strong>help.</strong>  : show this "
      "help<br/> ">>.
...
%% solution.
server_loop(P0, TreeEts, WebPid, TracePid,
	    TraceStatus) ->
    process_flag(trap_exit, true),
    put(?DYNAMIC_STRUCTS,
	common:get_logical_name(TreeEts, ?DYNAMIC_STRUCTS)),
    ParseGoal = (catch web_parse_code(P0)),
    ?CONSOLE_LOG(" get aim ~p", [ParseGoal]),
    {_, T, T1} = now(),
    MyResult = case ParseGoal of
		 {ok, halt} ->
		     WebPid ! {<<"No,aim is finished<br/>">>, finish};
		 {ok, trace_on} -> WebPid ! {?TRACE_ON, finish};
		 {ok, trace_off} -> WebPid ! {?TRACE_OFF, finish};
		 {ok, listing} ->
		     ResBin = prolog_shell:get_code_memory_html(TreeEts),
		     Footer = (?MSG_FOOTER),
		     WebPid !
		       {result, <<ResBin/binary, " aim is finished<br/>">>,
			finish, self()};
		 {ok, help} ->
		     ResBin = get_help(),
		     Footer = (?MSG_FOOTER),
		 ...
		 {ok, Goal = {':-', _, _}} ->
		     WebPid !
		       {result,
			<<"No, use online IDE for making rules,aim "
			  "is finished<br/>">>,
			finish, self()};
		 {ok, Goal} when is_tuple(Goal) ->
		     {TempAim, _ShellContext} =
			 prolog_shell:make_temp_aim(Goal),
		 ...
		 {ok, Goal} ->
		     WebPid !
		       {result,
			<<"No, use online IDE for making rules, "
			  "aim is finished<br/>">>,
			finish, self()};
		 {error, P = {_, Em, E}} ->
		     ...
		     NormalP = result(P),
		     WebPid !
		       {result, <<"No,aim is finished ~n", NormalP/binary>>,
			finish, self()};
		 Error ->
		     ...
		     WebPid !
		       {result,
			<<"No unexpected symbol,aim is finished "
			  "<br/>">>,
			finish, self()}
	       end,
...
web_parse_code(P0) ->
    {ok, Terms, _L1} =
	erlog_scan:string(unicode:characters_to_list(P0)),
    erlog_parse:term(Terms).
...
process_prove_erws(TempAim, Goal, Res, WebPid,
    ...
    case Res of
	  ...
	  MainRes = io_lib:format("No<br/> ~p, aim is finished",
				  [Reason]),
	  FinishTime = erlang:now(),
	  ElapsedTime = time_string(FinishTime, StartTime),
	  Main = concat_result([MainRes, ElapsedTime]),
	  ?CONSOLE_LOG("~p send back restul to web console ~p",
		       [{?MODULE, ?LINE}, {Main, WebPid}]),
	  WebPid ! {result, Main, finish, self()};
      false ->
	  MainRes = io_lib:format("No<br/>, aim is finished", []),
	  FinishTime = erlang:now(),
	  ElapsedTime = time_string(FinishTime, StartTime),
	  Main = concat_result([MainRes, ElapsedTime]),
	  WebPid ! {result, Main, finish, self()};
      {true, SomeContext, Prev} ->
	  ?CONSOLE_LOG("~p got from prolog shell aim ~p~n",
		       [?LINE, {ProtoType, SomeContext}]),
	  FinishTime = erlang:now(),
	  New = prolog_matching:bound_body(Goal, SomeContext),
	  ?CONSOLE_LOG("~p temp  shell cconontext ~p previouse "
		       "key ~p ~n",
		       [?LINE, New, Prev]),
	  {true, NewLocalContext} =
	      prolog_matching:var_match(Goal, New, dict:new()),
	  VarsRes = [shell_var_match_str(V1)
		     || V1 <- dict:to_list(NewLocalContext)],
	  ElapsedTime = time_string(FinishTime, StartTime),
	  ResStr = io_lib:format("Yes looking next ?", []),
	  Main = concat_result([VarsRes, ResStr, ElapsedTime]),
	  ?CONSOLE_LOG("~p got from prolog shell aim ~p~n",
		       [?LINE, {WebPid, VarsRes, ProtoType, NewLocalContext}]),
	  ...
	  receive
		...
		case Line of
		  finish ->
		      ...
		      WebPid !
			{result, concat_result(VarRes), finish, self()};
		  _ ->
		      ?CONSOLE_LOG("~p send next to pid ~n",
				   [{?MODULE, ?LINE}]),
...
shell_var_match_str({{Key}, []}) ->
    io_lib:format("<strong>~p</strong> = nothing ~n",
		  [Key]);
shell_var_match_str({{Key}, Val1}) ->
    Val = lists:flatten(erlog_io:write1(Val1)),
    ?CONSOLE_LOG("~p fill shell vars ~p",
		 [{?MODULE, ?LINE}, {Key, Val, Val1}]),
    case shell_check(Val) of
      true ->
	  io_lib:format("<strong>~p</strong> = ~ts ~n",
			[Key, Val]);
      false ->
...
shell_check([]) -> true;
shell_check([Head | Tail]) when Head < 20 -> false;
shell_check([Head | Tail]) when is_integer(Head) ->
    shell_check(Tail);
shell_check(_L) -> false.

time_string(FinishTime, StartTime) ->
    io_lib:format("<br/><span class='time'> elapsed time "
		  "~p secs </span><br/>",
		  [timer:now_diff(FinishTime, StartTime) *
		     9.99999999999999954748e-7]).
