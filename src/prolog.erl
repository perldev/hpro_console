...
delete_inner_structs()->
      ets:delete_all_objects(?INNER) ,
      ets:delete_all_objects(?RULES),
      ets:delete_all_objects(?META),
      ets:delete_all_objects(?DYNAMIC_STRUCTS),  
      ets:delete_all_objects(?META_LINKS),
      ets:delete_all_objects(?HBASE_INDEX),
      ets:delete_all_objects(?SCANNERS_HOSTS_TABLE),
      ets:delete_all_objects(?SCANNERS_HOSTS_LINK)
...
%%TODO realize separate namespaces
    ets:new(?DYNAMIC_STRUCTS,[named_table, set, public]),
    ets:new(?META,[named_table, set, public]),
    ets:new(?INNER,[named_table, bag, public]),
    ets:new(?RULES,[named_table, bag, public]),
    ets:new(?META_LINKS,[named_table, bag, public]),
...
dynamic_del_link(Fact1, Fact2) ->
     ?WAIT("~p regis  wait del link   ~p ~n",[{?MODULE,?LINE},{Fact1, Fact2} ]),

    receive 
	  start ->
	       ?WAIT("~p GOT  wait del link   ~p ~n",[{?MODULE,?LINE},{Fact1, Fact2} ]),
		List = ets:lookup(?META_LINKS, Fact1 ),
		ets:delete(?META_LINKS, Fact1 ), %delete links in memory
		lists:foreach(fun(  Val = { _Fact, Elem, _Val1 } )->
				  case Elem of
				      Fact2 -> next;
				      _ ->  ets:insert(?META_LINKS, Val)    
				  end
			      end,  List  ),	  
		fact_hbase:del_link_fact(Fact1, Fact2),
		exit(normal)
      end
...
dynamic_del_fact(B)->
	  ...
	  start ->
		...
		fact_hbase:del_fact(B)

      end
.

dynamic_del_rule(Search)->
	 ...
	 start->
	      ...
	      RuleList = ets:lookup(?RULES, Name),
	      %%get all prototypes
...
dynamic_new_rule(Tree = {':-',ProtoType, Body} , first )->
     ...
     case ets:lookup(?RULES, Name  )  of
	  []-> ets:insert(?RULES, New );
	  List ->
	       ets:delete(?RULES, Name),%%TODO replace it
	       ets:insert(?RULES,  [ New  | List]  )
	      
      end,
      ?DEBUG("~p result rule after insert ~p ~n",[{?MODULE,?LINE}, ets:lookup(?RULES, Name  ) ])
      
;
dynamic_new_rule(Tree = {':-',ProtoType, Body} , last )->
     Name  = element(1, ProtoType),
     Args =  common:my_delete_element(1,ProtoType),
     ?LOG("~n compile new rule last ~p ~n",[ Name  ]),
     New =  { Name, Args, Body },
     fact_hbase:add_new_rule(Tree, last ),
     ets:insert(?RULES,  New ),
     ?DEBUG("~p result rule after insert ~p ~n",[{?MODULE,?LINE}, ets:lookup(?RULES, Name  ) ])
;
dynamic_new_rule(SomeThing ,_ )->
	io:format("error ~p  ~n",[SomeThing]).


add_operator({op, OrderStatus, Type, Name }) when is_integer(OrderStatus)->
    case OrderStatus>1200 of 
	true -> false;
	false -> case OrderStatus<0 of
		    true-> false;
		    false->
			  add_operator(OrderStatus,Type, Name)
		end
    end
;
add_operator(_)->
...
add_operator(OrderStatus, yfx, Name   ) ->
     ets:insert(?DYNAMIC_STRUCTS, {Name, OrderStatus ,OrderStatus, OrderStatus-1  }  ),    
     true
;   
add_operator(OrderStatus, xfy, Name   )->  
     ets:insert(?DYNAMIC_STRUCTS, {Name, OrderStatus - 1 ,OrderStatus, OrderStatus  }  ),
     true
;
add_operator(OrderStatus, xfx, Name )->
     ets:insert(?DYNAMIC_STRUCTS, {Name, OrderStatus - 1 ,OrderStatus, OrderStatus-1  }  ),
     true
;
%prefix operators
add_operator(OrderStatus, fy, Name  )->
     ets:insert(?DYNAMIC_STRUCTS, {Name, OrderStatus, OrderStatus   }  ),
     true
;
add_operator(OrderStatus, fx, Name ) ->
     ets:insert(?DYNAMIC_STRUCTS, {Name, OrderStatus, OrderStatus - 1   }  ),
     true
;
% postfix
add_operator(OrderStatus, yf, Name ) ->
     ets:insert(?DYNAMIC_STRUCTS, {Name, OrderStatus ,OrderStatus  }  ),
     true
;
add_operator(OrderStatus, xf, Name  )->
     ets:insert(?DYNAMIC_STRUCTS, {Name, OrderStatus ,OrderStatus  }  ),
     true
;
add_operator(_OrderStatus, _, _Name  )->
     false
.
...
aim({ assertz, SourceFact, ForeignFact,  Body = {':-', _ProtoType, _Body1 }  }, Context, Index, _  ) ->
  aim( { assert,  SourceFact, ForeignFact,  Body   }, Context, Index, 0   )
;
aim({ assert, SourceFact,  ForeignFact, Body = {':-', ProtoType, _Body1 }  }, Context, _Index, _ ) ->
      dynamic_new_rule(Body, last),
      RuleName = erlang:element(1, ProtoType),
      fact_hbase:add_link(SourceFact, ForeignFact, RuleName ),
      ets:insert(?META_LINKS,{SourceFact, ForeignFact, RuleName  }),
      {true, Context} 
;

aim({ assertz, SourceFact, ForeignFact,  RuleName  }, Context, Index, _  ) ->
  aim( { assert,  SourceFact, ForeignFact,  RuleName   }, Context, Index, 0   )
;

aim({ assert, SourceFact,  ForeignFact, RuleName }, Context, _Index, _ ) ->
      fact_hbase:add_link(SourceFact, ForeignFact, RuleName ),
      ets:insert(?META_LINKS,{SourceFact, ForeignFact, RuleName  }),
      {true, Context}
;
%%%usual assert
aim({ assertz, Body = {':-', _ProtoType, _Body1 }  }, Context, Index, _  ) ->
  aim( { assert, Body   }, Context, Index, 0   )
;
aim({ assert, Body = {':-', _ProtoType, _Body1 }  }, Context, _Index, _ ) ->
      dynamic_new_rule(Body, last),
      {true, Context}

;
aim({ asserta, Body = {':-', _ProtoType, _Body1 }   }, Context, _Index, _  )->
     dynamic_new_rule(Body, first),
     {true,Context}
    
;
aim({ assertz, Body   }, Context, Index, _  ) when is_tuple(Body)->
    aim( { assert, Body   }, Context, Index, 0   )
;
aim({ assert, Body   }, Context, _Index, _  ) when is_tuple(Body)->
%       Name = element(1,Body),
      BodyBounded = bound_aim(Body, Context),
      case is_rule(BodyBounded) of 
	    true -> 
		    dynamic_new_rule(  {':-', BodyBounded, true }, last);
	    false ->
		    link_fact(BodyBounded ),
		    fact_hbase:add_new_fact( BodyBounded, last)
      end,
      ?DEBUG(" add ~p  yes ~n",[Body ]),
      {true,Context}
;
aim({ asserta, Body   }, Context, _Index, _  ) when is_tuple(Body)->
%       Name = element(1,Body),
      BodyBounded = bound_aim(Body, Context),
      case is_rule( BodyBounded ) of 
	    true -> 
		    dynamic_new_rule(  {':-', BodyBounded, true }, first);
	    false ->
		    link_fact( BodyBounded ),
		    fact_hbase:add_new_fact(BodyBounded, first)
      end,
      ?LOG(" add ~p  yes ~n",[Body ]),
      {true, Context}
;
aim({ retract, FirstFact, SecondFact } , Context, _Index, _  ) when is_atom(FirstFact)->
	Pid = spawn(?MODULE, dynamic_del_link, [ FirstFact, SecondFact ] ),
	unlink(Pid),
	Pid ! start,
...
aim(Body = { op, _OrderStatus, _, Name }, Context, _Index, _TreeEts  ) when is_atom(Name) ->
    Res = add_operator( Body ),
    {Res, Context}
;   
...
%%work low level api for incoming
...
link_fact(Body)->
  link_fact(Body, ?SIMPLE_HBASE_ASSERT)
.

link_fact( Body, 1 )->
    worker_linked_rules( Body)
...
linked_loop(FactName, RuleCall)->
      ...
      receive
	    {result, Result } -> 
		?LOG("~p got result ~p from aim ~p for ~p ~n",[{?MODULE,?LINE}, Result, 
								RuleCall, FactName] ),
	       	true						
		;
...
start_rule(ProtoType, Context, Index, TreeEts, ParentPid)->
	   ...
	   converter_monitor:stat(search_rule,  Name , Search, true ),
	   RuleList = ets:lookup(?RULES, Name),
	    %%get all prototypes
	   ...
	   MatchedRules  = lists:foldr(fun(E, Hash)->  
	    ...
	    ChildPid = receive 
			  start ->
			      ...
			      spawn_link(?MODULE, start_matched_rules, [Name, MatchedRules, erlang:self(), NewIndex, TreeEts] )
			end,
	    ChildPid ! start,
...
;
% bound_struct_hack(Var) when is_list(Var)-> %TODO 
%     BoundVar = 
% 	      lists:map(fun(E)->
% 			    case prolog_matching:is_var(E) of
% 			      true -> process_var_hack(E);
% 			      _-> E
% 			    end
% 			 end , Var),
%     BoundVar
...
operators( Op )->
  case ets:lookup(?DYNAMIC_STRUCTS, Op) of
      [ {Op, Status1, Status2 } ] -> 
...
start_retract_process(BodyBounded, Context, Index, TreeEts, ParentPid) when is_tuple(BodyBounded)->
	...
	next ->
		...
		case is_deep_rule( Name ) of
			...
			Pid = spawn(?MODULE, dynamic_del_rule, [BodyBounded]),
 		    	unlink(Pid),
		        Pid ! start,
		     ...
		     false->
		         Pid = spawn(?MODULE, dynamic_del_fact, [BodyBounded] ),
			 unlink(Pid),
			 Pid ! start,
