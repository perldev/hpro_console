...
%       prolog:compile("pro.pl"),
      ...
      ScannerUrlCacheMeta  = get_scanner(?META_FACTS, ScannerCache),
      
      ScannerUrlAiMeta  = get_scanner(?META_FACTS, ScannerAiMeta),
      ScannerUrlMeta  = get_scanner(?META_FACTS, ScannerMeta),
      ScannerUrl  = get_scanner(?RULES_TABLE, Scanner),

      ?DEBUG("~p get scanner url ~p ~n",[{?MODULE,?LINE}, ScannerUrl]),
      get_cache_meta_info(ScannerUrlCacheMeta),
      get_link_meta_info(ScannerUrlAiMeta),
      get_meta_facts(ScannerUrlMeta),
   
      get_and_load_rules(ScannerUrl,?RULES)
.

get_cache_meta_info([])->
    ?LOG("~p empty scanner~n",[{?MODULE,?LINE}])
;
get_cache_meta_info(Scanner)->
...
%  	 httpc:set_options( [ {verbose, debug} ]),
	Host = get_host(Scanner),
        case catch  httpc:request( get, { Scanner,[ {"Accept","application/json"}, {"Host", Host}]},
			    ...
			    process_cache_link( Text1 ),
			    get_cache_meta_info(Scanner);%% TODO another solution
...
process_cache_link([])-> 
  []
;	
process_cache_link(Meta)  when is_list(Meta)-> 
  process_cache_link( list_to_binary(Meta) )
;
process_cache_link(Meta)->
	...
	lists:foreach(fun(Elem)->
			...
			lists:foreach( fun(E)-> 
					  ...
					  ?DEBUG("~p insert cache info ~p ~n",[ {?MODULE,?LINE}, 
										   { RowName, Col, Val  } ] ),
					  ets:insert(?HBASE_INDEX, { RowName, Col, Val  })
					end, 
				      RowCells) 
		      end, Cache )
...
%  	 httpc:set_options( [ {verbose, debug} ]),
	...
	Host = get_host(Scanner),
        case catch  httpc:request( get, { Scanner,[ {"Accept","application/json"}, {"Host", Host}]},
				    ...
				    [ {sync, true} ] ) of
                    { ok, { {_NewVersion, 200, _NewReasonPhrase}, _NewHeaders, Text1 } } ->
			    process_meta_link( Text1 ),
			    get_link_meta_info(Scanner);%% TODO another solution
...
process_meta_link([])-> 
    ...
    []
;
process_meta_link(Meta)  when is_list(Meta)-> 
  process_meta_link( list_to_binary(Meta) )
;
process_meta_link(Meta)->
       Json = jsx:decode( Meta ),
	...
	lists:foreach(fun(Elem)->
			...
			lists:foreach( fun(E)-> 
					  ...
					  ets:insert(?META_LINKS, { RowName, Col, Val  })
					end, RowCells) 
		      end, Links )
...
%  	 httpc:set_options( [ {verbose, debug} ]),
	Host = get_host(Scanner),
        case catch  httpc:request( get, { Scanner,[ {"Accept","application/json"}, {"Host", Host}]},
                    ...
                    { ok, { {_NewVersion, 200, _NewReasonPhrase}, _NewHeaders, Text1 } } ->
			    process_meta( Text1 ),
			    get_meta_facts(Scanner);%% TODO another solution
...
process_meta([] )->
     []
; 
process_meta(Meta ) when is_list(Meta)->
      process_meta(  list_to_binary(Meta) )
;   
process_meta(Meta )->
      Json = jsx:decode( Meta ),
      ?DEBUG("~p got meta ~p ~n",[ {?MODULE,?LINE}, Json ] ),
      [{_Row, Facts }] = Json,
      lists:foreach(fun process_hbase_meta_fact/1, Facts )
.
process_hbase_meta_fact(Fact)->
%       [{<<"key">>,<<"aXA=">>},
%                               {<<"Cell">>,
...
%                                  {<<"$">>,<<"bWQ1">>}]]}]
      ...
      ets:insert(?META, {Name, list_to_integer(Count), Func })
.
...
%       Search =  common:my_delete_element(1, Aim),%%get prototype
	  ...
	  start ->
		...
		[]-> 
			...
			[]->
			      case check_exist_facts( Name ) of %%and try to find in hbase
				true-> 
...
fact_start_link_inner( Aim, Key, TreeEts, ParentPid )->
      ...
      FactList = ets:lookup(?INNER, Name),
...
fact_start_link_hbase( Aim, Key, TreeEts,  ParentPid )->
      ...
      case check_params_facts(Name) of
	  {CountParams, HashFunction} ->
		 case  check_index(ProtoType, Name) of
		      []->
			    {Scanner, List } = start_recr( atom_to_list(Name) , ProtoType  ),
			     case List of
				[] -> 
		       ...
		       {IndexTable , PartKey } ->
			      ?DEBUG("~p got index ~p ~n",[{?MODULE,?LINE}, {IndexTable , PartKey } ]),
				...
				_ -> 
			      ...
			      process_indexed_hbase(atom_to_list(Name), ProtoType, Key, PreRes,  TreeEts)
		end;
	  Res->
...
check_index(ProtoType, Name)->
	...
	PossibleKey = string:join( lists:reverse(FindKey), ","),
	 Indexes = ets:lookup(?HBASE_INDEX, Name),
	...
	case   PossibleKey of
	    ...
	    _ ->
		case ets:lookup(?HBASE_INDEX, Name) of
		  [] -> [];
		  List ->
...
add_new_fact(Body, Pos)->
    add_new_fact(Body, Pos, ?SIMPLE_HBASE_ASSERT)    
.

add_new_fact(Body, Pos, 1) when is_tuple(Body)->
    add_new_fact(tuple_to_list(Body), Pos, 1)
;
add_new_fact([ Name | ProtoType ] , Pos, 1)->
	  ?DEBUG("~p new fact there ~p ~n",[{?MODULE,?LINE}, [ Name | ProtoType ]  ]),
	  CountParams = length(ProtoType),
	  converter_monitor:stat(try_add,  Name , ProtoType, true ),
	  case check_params_facts( Name ) of
	      {CountParams, _HashFunction } ->
		  Params = prototype_to_list(ProtoType),
		  index_work_add(Name, Params),%%TODO spawn this
		  Res = store_new_fact(Name, Params ),
		  converter_monitor:stat(add,  Name , ProtoType, Res ),
		  Res;
		  ...
		  ;
	      false -> 
		  create_new_fact_table(Name),
		  ets:insert(?META, {Name, CountParams, "md5"} ),
		  store_meta_fact(Name, [
					  {"count" ,integer_to_list( CountParams) }, 
					  {"hash_function","md5" },
					   {"facts_count", "1" }
					  ]  ),%TODO NEW HASH function
		  Res = store_new_fact(Name , prototype_to_list(ProtoType) ),
		  converter_monitor:stat(add, Name, ProtoType, Res ),
		  Res
	  end
;
add_new_fact(Body, Pos, Something) when is_list(Body)->
    add_new_fact(list_to_tuple(Body), Pos, Something)
;
add_new_fact(Body, last, Something) ->
    ets:insert(?INNER, Body)
;
add_new_fact(Body, first, Something) ->
...
index_work_add(Name, Params)->
     Key = generate_key( Params ),
     case  ets:lookup(?HBASE_INDEX, Name) of
	   [] -> [];
	   List ->
		  lists:foreach(fun({Key, IndexTable, KeyName})->
...
index_work_del(Name, Params) when is_tuple(Params)->
   index_work_del(Name, tuple_to_list(Params)) 
;
index_work_del(Name, Params)->
     Key = generate_key( Params ),
     case  ets:lookup(?HBASE_INDEX, Name) of
	   [] -> [];
	   List ->
...
check_params_facts(Name) when is_list(Name)->
      check_params_facts( list_to_atom(Name) )


;
check_params_facts(Name) ->
...
check_exist_facts(Name) when is_list(Name)->
      check_exist_facts( list_to_atom(Name) )


;
check_exist_facts(Name) ->
     case ets:lookup(?META, Name  ) of
	  [ Var ] -> true;
	  []-> false
     end
...
add_new_rule(Tree = { ':-' ,ProtoType, BodyRule}, Pos )->
 add_new_rule(Tree, Pos, ?RULES_TABLE, ?SIMPLE_HBASE_ASSERT )
.
...
del_link_fact(Fact1,Fact2) when is_atom(Fact1) ->
	del_link_fact(atom_to_list(Fact1), Fact2)
;
del_link_fact(Fact1, Fact2) when is_atom(Fact2) ->
	del_link_fact(Fact1, atom_to_list(Fact2) )
;
del_link_fact(Fact1, Fact2)->
      del_key(Fact1, ?META_FACTS, ?LINKS_FAMILY, Fact2 )
.
del_fact(B)->
  del_fact(B, ?SIMPLE_HBASE_ASSERT)
.
del_fact(B,1)->
      Name = erlang:element(1,B),
      ProtoType = common:my_delete_element(1,B),
      converter_monitor:stat(try_del,  Name , ProtoType, true ),
      index_work_del(Name, ProtoType),
      Res = del_key( generate_key(ProtoType), Name),
      converter_monitor:stat(del,  Name , ProtoType, Res),
      Res
...
del_rule(Rule)->
  del_rule(Rule, ?SIMPLE_HBASE_ASSERT)    
.
del_rule(Rule, 1)->
  del_key(Rule, ?RULES_TABLE);
del_rule(_,_)->
  true.
	    ...
	    {Hbase_Res, Host } = get_rand_host(),
            case catch  httpc:request( post, { Hbase_Res++LTableName++"/"++Key++"/"++?FAMILY,
			 ...
			 { ok, { {_NewVersion, 200, _NewReasonPhrase}, _NewHeaders, Text1 } } ->
				...
				true;
                        Res ->
                            ?WAIT("~p got from hbase ~p ~n",[?LINE,{Res,Name,LProtoType }]),
                            false
